...
def _preprocess_yolo(img, input_shape, letter_box=True):
...
def _postprocess_yolo_batch(trt_outputs, img_w, img_h, conf_th, nms_threshold,
                      input_shape, letter_box=False):
    ...
    indexes = []
    for bi in range(bn):
        boxes, scores, classes = _postprocess_yolo([out[bi] for out in trt_outputs], img_w, img_h, conf_th, nms_threshold,
                      input_shape, letter_box=False)
...
def do_inference(context, bindings, inputs, outputs, stream, batch_size=2):
...
class TrtYOLO(object):
    ...
    def detect_batch(self, batch, conf_th=0.3, letter_box=None):
        """Detect objects in the input image."""
        letter_box = self.letter_box if letter_box is None else letter_box
        resized_batch = [_preprocess_yolo(img, self.input_shape, letter_box) for img in batch]
        resized_batch = np.stack(resized_batch)

        bn, channels, net_h, net_w = resized_batch.shape
        assert resized_batch.nbytes == (3 * net_h * net_w * np.dtype('float32').itemsize * len(batch))
        # Set host input to the image. The do_inference() function
        # will copy the input to the GPU before executing.
        self.inputs[0].host = np.ascontiguousarray(resized_batch)
        if self.cuda_ctx:
            self.cuda_ctx.push()
        trt_outputs = self.inference_fn(
            context=self.context,
